...
from streamlit_drawable_canvas import st_canvas # Updated import
import numpy as np
import pandas as pd
...
st.markdown("""
1. **Upload** your map.
2. **Click** the spots where you measured signal. 
3. **Enter** the dBm value for each click in the table.
4. **Identify** which point is your Router.
""")
            ...
            with col2:
                ...
                if st.button("🚀 Calculate & Show Range"):
                    ...
                    if len(meas_df) >= 1:
                        ...
                        grid_x, grid_y = np.meshgrid(np.linspace(0, display_width, 150), 
                        ...
                        fig = px.imshow(grid_signals, color_continuous_scale='RdYlGn', 
                                        range_color=[-90, -30], labels={'color':'dBm'})
